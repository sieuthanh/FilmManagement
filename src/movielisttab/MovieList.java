...
public class MovieList extends JPanel implements ActionListener {
    ...
    private Object[] colname = new Object[] {"Title","Country","Year","Rating","Time", "Id"};
    private Object[][] data = {};
    ...
    private DefaultTableModel model = new DefaultTableModel(data, colname) {
            ...
            @Override//disable table's editable mode
    ...
    public MovieList() {
        ...
        for (int i = 0; i < list.size(); i ++) {
            ...
            model.addRow(new Object[]{movie.getName(), movie.getCountry(),
            movie.getYear(), movie.getRating(), movie.getRuntime(), movie.getId()});
        }
        ...
        for (int i = 0; i <= 4; i ++) {
            ...
            switch (i) {
                case 0: col.setPreferredWidth(365); break;
                case 1: col.setPreferredWidth(95); break;//75
                case 2: col.setPreferredWidth(75); break;
                case 3: col.setPreferredWidth(90); break;
                case 4: col.setPreferredWidth(75); break;
            }
        }
        ...
        table.getTableHeader().addMouseListener(new MouseAdapter(){
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                clearTable(model);
                if (previousOrderedColumn == column) currentOrderedCommand = 1 - previousOrderedCommand;
                else {
                ...
                switch (column) {
                    case 0: {
                        orderedList = database.getMovieListByOrder("name", currentOrderedCommand);                        
                        break;
                    }
                    case 1:  {
                        orderedList = database.getMovieListByOrder("country", currentOrderedCommand);
                        break;
                    }
                    case 2: {
                        orderedList = database.getMovieListByOrder("year", currentOrderedCommand);
                        break;
                    }
                    case 3: {
                        orderedList = database.getMovieListByOrder("rating", currentOrderedCommand);
                        break;
                    }
                    case 4: {
                        orderedList = database.getMovieListByOrder("runtime", currentOrderedCommand);
                        break;
                    }
                }
        ...
        scrollpane.setBounds(insets.left + 10, insets.top + 50, d.width, d.height);
        add(scrollpane);
    ...
    public final void CharClassifyInitilaize() {
        ...
        charclassify[1].setToolTipText("Show movies that their titles start by A, B, C or D");
        //
        charclassify[2] = new CharClassify("E F G H");
        charclassify[2].setToolTipText("Show movies that their titles start by E, F, G or H");
        //
        charclassify[3] = new CharClassify("I J K L");
        charclassify[3].setToolTipText("Show movies that their titles start by I, J, K or L");
        //
        charclassify[4] = new CharClassify("M N O P");
        charclassify[4].setToolTipText("Show movies that their titles start by M, N, O or P");
        //
        charclassify[5] = new CharClassify("Q R S T");
        charclassify[5].setToolTipText("Show movies that their titles start by Q, R, S or T");
        //
        charclassify[6] = new CharClassify("U V W");
        charclassify[6].setToolTipText("Show movies that their titles start by W, V, or W");
        //
        charclassify[7] = new CharClassify("X Y Z");
        charclassify[7].setToolTipText("Show movies that their titles start by X, Y, or Z");
        //
        charclassify[8] = new CharClassify("Others");
        charclassify[8].setToolTipText("Show movies that their titles start by sepecial characters like"
                + " 0-9, /, (, ), etc");
        //add all buttons into panel
        for (int i = 0; i<= 8; i ++) {
            Dimension d = charclassify[i].getPreferredSize();
            charclassify[i].setBounds(insets.left + 10 + i * 69, insets.top + 20, d.width, d.height);
            add(charclassify[i]);
        }
        //add action listener to those buttons
        for (int i = 0; i <= 8; i ++) charclassify[i].addActionListener(this);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        int i;
        for (i = 0; i <= 8; i ++)
            if ((e.getSource() == charclassify[i]) && !charclassify[i].isActive()) {
                for (int j = 0; j <= 8; j ++)
                    if (charclassify[j].isActive()) {
                        charclassify[j].setActive(false);
                        charclassify[j].setFont(new Font("Tahoma", Font.PLAIN, 15));
                    }
                currentbutton = i;

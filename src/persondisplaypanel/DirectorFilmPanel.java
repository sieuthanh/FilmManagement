 ...
 * @author uNstOppAbLe
 */
public class DirectorFilmPanel extends JPanel {

    private Insets insets = getInsets();
    private Dimension d;
    private JLabel thumblabel;
    private MovieNameLabel label;
    private List moviebydirector = new LinkedList();
    ...
    public DirectorFilmPanel (String id)
    {
        setLayout(null);
        moviebydirector = new DatabaseManager().getMovieByDirector(id);
        n = moviebydirector.size();

        if (!moviebydirector.isEmpty())
        {
            for (int i=0; i<n; i++)
            {
                movie = (Movie)moviebydirector.get(i);
